...
def compound_interest_calculator(P, r, n, t):
    ...
    try:
        # Check if inputs are valid
        if not all(isinstance(arg, (int, float)) for arg in (P, r, n, t)) or r < 0 or n <= 0 or t < 0:
            raise ValueError("Invalid input. Please provide valid numeric values.")

        # Calculate compound interest using a different formula A = P * e^(rt)
        A = P * (2.71828) ** (r * t)
        return A
